...
const ACE_XZ: &[u8] = include_bytes!("ace.js.xz");
const MODE_GLSL: &str = r#"
...
const THEME_TOMORROW_NIGHT: &str = r#"
...
static ACE: OnceLock<Ace> = OnceLock::new();

#[derive(Debug)]
pub(crate) struct AceEditor<'a> {
    inner: JsValue,
    parent: &'a JsValue
}

impl<'a> AceEditor<'a> {
    fn get_session(&self) -> JsValue {
        Reflect::get(&self.inner, &JsValue::from_str("session")).unwrap()
    }

    pub(crate) fn set_theme(&self, theme: &str) {
        Function::from(Reflect::get(&self.inner, &JsValue::from_str("setTheme")).unwrap())
            .call1(&self.inner, &JsValue::from_str(theme)).unwrap();
    }

    pub(crate) fn set_mode(&self, mode: &str) {
        let module = Function::from(Reflect::get(self.parent, &JsValue::from_str("require")).unwrap())
            .call1(&self.parent, &JsValue::from_str(mode)).unwrap();
        let mode_obj = Reflect::get(&module, &JsValue::from_str("Mode")).unwrap();
        let session = self.get_session();
        let set_mode = Reflect::get(&session, &JsValue::from_str("setMode")).unwrap();
        // class constructor hack
        let cls = Function::new_with_args("c", "return new c();")
            .call1(&JsValue::undefined(), &mode_obj).unwrap();
        Function::from(set_mode).call1(&session, &cls).unwrap();
    }

    pub(crate) fn set_value(&self, value: &str) {
        Function::from(Reflect::get(&self.inner, &JsValue::from_str("setValue")).unwrap())
            .call1(&self.inner, &JsValue::from_str(value)).unwrap();
    }

    pub(crate) fn set_readonly(&self, value: bool) {
        Function::from(Reflect::get(&self.inner, &JsValue::from_str("setReadOnly")).unwrap())
            .call1(&self.inner, &JsValue::from_bool(value)).unwrap();
    }

    pub(crate) fn select_none(&self) {
        let session = self.get_session();
        let selection = Reflect::get(&session, &JsValue::from_str("selection")).unwrap();
        let clear_selection = Function::from(Reflect::get(&selection, &JsValue::from_str("clearSelection")).unwrap());
        clear_selection.call0(&selection).unwrap();
    }
}
...
impl Ace {
    pub(crate) fn edit(&self, elem_id: &str) -> AceEditor {
        AceEditor {
            inner: Function::from(Reflect::get(&self.module, &JsValue::from_str("edit")).unwrap())
...
// should be fine on the wasm rt
unsafe impl Sync for Ace {}
unsafe impl Send for Ace {}

pub fn get_ace<'a>() -> &'a Ace {
    if let Some(m) = ACE.get() {
        return m;
    }
    let start = now();
    let mut res = Vec::new();
    xz_decompress(&mut ACE_XZ, &mut res).unwrap();
    let script = String::from_utf8(res).unwrap();
    let func = Function::new_no_args(&format!("{};return ace;", script));
    let ace = func.call0(&JsValue::undefined()).unwrap();

    let init_extensions = Function::new_no_args(&format!("{MODE_GLSL};{THEME_TOMORROW_NIGHT};"));
    init_extensions.call0(&JsValue::undefined()).unwrap();

    ACE.set(Ace { module: ace }).unwrap();

    let elapsed = now() - start;
    info!("Loaded Ace in {} ms", elapsed);
    ACE.get().unwrap()
}
