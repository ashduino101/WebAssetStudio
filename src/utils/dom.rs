...
use web_sys::{Blob, BlobPropertyBag, Element, Url};
use crate::utils::tex::pngenc::encode_png;

pub fn create_data_url(data: &[u8], mimetype: &str) -> String {
    let arr = Uint8Array::new_with_length(data.len() as u32);
    arr.copy_from(&data);
    let parts = Array::new_with_length(1);
    parts.set(0, JsValue::from(arr));
    let props = BlobPropertyBag::new();
    props.set_type(mimetype);
    let blob = Blob::new_with_u8_array_sequence_and_options(&JsValue::from(parts), &props).expect("failed to create blob");
